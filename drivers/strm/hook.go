...
func UpdateLocalStrm(parent string, objs []model.Obj) {
	path := utils.FixAndCleanPath(parent)
	updateLocal := func(driver *Strm, basePath string, objs []model.Obj) {
		...
		for _, obj := range objs {
			...
			generateStrm(driver, obj, localPath)
		}
	...
	_ = strmTrie.VisitPrefixes(patricia.Prefix(path), func(needPathPrefix patricia.Prefix, item patricia.Item) error {
		...
		for _, strmDriver := range strmDrivers {
			strmObjs := strmDriver.convert2strmObjs(context.Background(), stdpath.Join(stdpath.Base(needPath), restPath), objs)
...
func generateStrm(driver *Strm, obj model.Obj, localPath string) {
	if !obj.IsDir() {
		if utils.Exists(localPath) && driver.SaveLocalMode == SaveLocalInsertMode {
			return
		}
		link, err := driver.Link(context.Background(), obj, model.LinkArgs{})
		if err != nil {
			log.Warnf("failed to generate strm of obj %s: failed to link: %v", localPath, err)
			return
		}
		var data []byte
		if link != nil && link.MFile != nil {
			_, err = link.MFile.Seek(0, io.SeekStart)
			if err != nil {
				log.Warnf("failed to seek strm content %s: %v", localPath, err)
				link.MFile.Close()
				return
			}
			data, err = io.ReadAll(link.MFile)
			link.MFile.Close()
			if err != nil {
				log.Warnf("failed to read strm content %s: %v", localPath, err)
				return
			}
		} else {
			log.Warnf("failed to generate strm of obj %s: no MFile in link", localPath)
			return
		}

		same, err := isSameContent(localPath, int64(len(data)), bytes.NewReader(data))
		if err != nil {
		...
		if _, err := file.Write(data); err != nil {
			log.Warnf("failed to generate strm of obj %s: write failed: %v", localPath, err)
		}
	}
}
